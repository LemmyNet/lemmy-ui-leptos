...
export async function loginLogoutTest ({ page }) {
  ...
  await expect(page.locator("a").getByText("Login", { exact: true }).first()).toHaveText(
    "Login",
  );
  ...
  await expect(page.locator("summary").getByText("lemmy", { exact: true })).toBeVisible();
...
export async function persistLanguageTest ({ page }) {
  ...
  await expect(page.locator("summary").getByText("Lang", { exact: true })).toBeVisible();
...
export async function persistThemeTest ({ page }) {
  ...
  await expect(page.locator("summary").getByText("Theme", { exact: true })).toBeVisible();
